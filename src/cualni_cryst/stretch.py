...
def principal_stretches(U: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    vals, vecs = np.linalg.eigh(
        0.5 * (np.asarray(U, float) + np.asarray(U, float).T)
    )
...
def optimal_match_matrix_family(
    ...
    The previous repository check only required each matrix to have *some*
    nearby matrix in the other set. That can hide duplicate assignments.
    Here the Hungarian algorithm enforces a genuine one-to-one match.
    """
