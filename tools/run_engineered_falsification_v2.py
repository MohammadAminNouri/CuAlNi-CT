...
STRUCTURAL_TESTS = (
    ...
    "tests/test_equation_preserving_adaptive_core_v3.py",
...
def main():
    ...
    report = to_jsonable(
        {
            "campaign": "ENGINEERED_FALSIFICATION_V3_EQUATION_PRESERVING",
    ...
    report["status"] = (
        "PASS"
        if harness["passed"] and pytest.returncode == 0 and before == after
        else "FAIL"
    )
    ...
    if pf:
        print(
            ...
            {k: pf[0].get(k) for k in ("case_id", "contract", "failure_class", "residual", "allowed")},
        )
    if mf:
        first = mf[0]
